...
def signup_post():
    ...
    user = User.query.filter_by(
        email=email).first()  # will return a user if they already exist in database

    if user:  # if a user is found, we want to redirect back to signup page so user can try again
        flash('Email address already exists')
        return redirect(url_for('auth.signup'))

    # create a new user with the form data. This hashes the password so the plaintext version isn't saved.
